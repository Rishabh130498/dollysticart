    ...
    const initPage = async () => {
      ...
      try {
        ...
        if (secData && secData.draft_content && Object.keys(secData.draft_content).length > 0) {
          setContent({ ...DEFAULT_PRIVACY_CONTENT, ...secData.draft_content });
        } else if (secData) {
          ...
          setContent(newRow?.draft_content ? { ...DEFAULT_PRIVACY_CONTENT, ...newRow.draft_content } : DEFAULT_PRIVACY_CONTENT);
        }
      } catch (err) {
    ...
    try {
      ...
      if (!updateData || updateData.length === 0) {
        ...
        if (!insertData || insertData.length === 0) {
          throw new Error('Failed to save privacy policy draft to database.');
        }
      }
      setSaveStatus('saved');
      router.refresh();
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (!updateData || updateData.length === 0) {
        ...
        if (insertErr) throw insertErr;
        if (!insertData || insertData.length === 0) {
          throw new Error('Failed to publish privacy policy to database.');
        }
      }
      await revalidateCmsPaths(['/privacy', '/admin/privacy']);
      setSaveStatus('saved');
      router.refresh();
